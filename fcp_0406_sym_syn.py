...
class FCP_Hawkes(object):
    ...
    def __init__(self, b_prior_mu, b_prior_sigma, zeta_prior_mu, zeta_prior_sigma, eta_prior_mu, eta_prior_sigma,
        ...
        self.c_time[2,1] = 20
        self.c_time[3,0] = 20
        self.c_time[3,1] = self.T
        
        
        self.path_entity[0].extend([0,1,3])
        self.path_entity[1].extend([0,1,3])
        self.path_entity[2].extend([0,1,3])
        self.path_entity[3].extend([1,2,3])
        self.path_entity[4].extend([0,2,3])
        self.path_entity[5].extend([0,2,3])
        self.path_entity[6].extend([0,2,3])
        self.path_entity[7].extend([0,2,3])
        
        for i in range(self.d_num):
            self.entity_list[0].append(i)
            self.entity_list[3].append(i)

        for i in range(0,int(self.d_num/2)):
            self.entity_list[1].append(i)

        for i in range(int(self.d_num/2),self.d_num):
            self.entity_list[2].append(i)
            ...
            for p in exist:
                for q in exist:
                    if p == q:
                        ...
                        self.Hawkes_eta[p,q] = -10
                        self.Hawkes_zeta[p,q] = 10
                    else:
                        self.Hawkes_b[p,q] = -10
                        self.Hawkes_eta[p,q] = -10
                        self.Hawkes_zeta[p,q] = 10
        ...
        for i in range(self.d_num):
            ...
            self.path_entity[i].append(c_index)
        ...
        for p in index:
            for q in index:
                ...
                '''
                self.Hawkes_eta[q,p] = 10
                self.Hawkes_zeta[q,p] = 10
                '''
    ...
    def sample_z(self, sample_index):
        ...
        for i in range(self.d_num):
            ...
            # print(self.path_entity,'self.path entity in sample_z')

            # for exist in self.FCPList_exist:
            #     print(exist,'exist')
                
            #     for m in exist:
            #         for n in exist:
            #             print(np.around(logit(self.Hawkes_b[m,n])),
            #             np.around(logit(self.Hawkes_eta[m,n])),
            #             np.around(logit(self.Hawkes_zeta[m,n])),m,n,
            #             'Hawkes_b,_eta,_zeta[m,n],m,n in sample z')
            # if i==0 and sample_index == 8:
    ...
    def sample_hawkes(self, para_select, c_send, c_receive):
                ...
                if i!=j:
                    ...
                    if para_select == 0:
                        ...
                        for m in range(len(r_array)):
                            if s_array[m] == c_send and r_array[m] == c_receive:
        ...
        print(np.around(self.b*logit(new_b)),np.around(self.eta*logit(new_eta)),
        np.around(self.zeta*logit(new_zeta)), np.around(loglike_),
        'new_b,new_eta,new_zeta, q_')
        print(np.around(self.b*logit(self.Hawkes_b[c_send,c_receive])),
        np.around(self.eta*logit(self.Hawkes_eta[c_send,c_receive])),
        np.around(self.zeta*logit(self.Hawkes_zeta[c_send,c_receive])),
        np.around(loglike),'old_b,old_eta,old_zeta, q')
        if para_select == 0:
            ...
            self.Hawkes_b[c_receive,c_send] = new_sample
        ...
        if para_select == 1:
            ...
            self.Hawkes_eta[c_receive,c_send] = new_sample
        ...
        if para_select == 2:
            ...
            self.Hawkes_zeta[c_receive,c_send] = new_sample
    ...
    def smc_weight(self, t_begin_index, t_end_index, i, path_i, empty_para_array,d):  
        ...
        time_ = copy.copy(self.FCPList_time)
        time_.append(self.T)
        ...
        for j in range(self.d_num):
            if j!=i:
                ...
                self.derive_info_hawkes_smc(i, j, time_, t_begin_index, t_end_index, path_i[0:t_end_index], self.path_entity[j][0:t_end_index], empty_para_array,d)
                
                loglike = loglike + self.calculate_hawkes_smc(time_, t_begin_index, t_end_index, b, 
    ...
    def derive_info_hawkes_smc(self, s, r, time, t_begin_index, t_end_index, path_s, path_r, empty_para_array,d):
        ...
        z_ij_like_index = np.searchsorted(time,x_ij_like,side='right')-1

        b_like = b[z_ij_like_index]
        ...
        return b_like, b, eta_ji_prior, zeta_ji_prior, eta_ji_like, zeta_ji_like, x_ij_like, x_ji_prior, x_ji_like
    ...
    b_base): 
        ...
        for t in range(t_begin_index,t_end_index):
            ...
            b_t = b_base[t]
            s = s-b_t*(t_e-t_b) 
            # print(t_b,t_e,b_t,'t_b,t_e,b_t')
        # calculate kernel likelihood in second part
        # print(s,'b likelihood in second part')
        ...
        # print(s,'likelihood in second part')
        # calculate the first part
        # print(b[0:len(x_ij)],'b[0:len(x_ij)]')
        # print(len(b),'len(b)')
        # print(len(x_ij))
        ...
        # print(b_base,s,'claculate hawkes smc')
    ...
    def smc_z(self, i, jump_array): 
        ...
        for t in range(len(jump_array_index)):
            ...
            if t==0:
                ...
                # print(t,'t in smz_z')
                # print(particle_path,'particle path')
                empty_array = np.zeros(self.particle_num)
                for d in range(self.particle_num):
                    if particle_path[d,jump_array_index[t]] == self.c_max:
                        empty_array[d] = 1
                        self.sample_empty_parameter_smc(particle_path,empty_para_array,jump_array_index[t],jump_array_index[t]+iter_len+1,d,jump_array)
                    else:
                        has_empty = np.where(particle_path[d,:]==self.c_max)[0]
                        if len(has_empty) != 0:
                            empty_array[d] = 1
                    # print(np.where(empty_para_array[:,:,0,0,d]>self.undefine_para_value),'np.where(empty_para_array[:,:,0,0,d]>self.undefine_para_value)')
                not_empty = np.where(empty_array==0)[0]
                is_empty = np.where(empty_array==1)[0]
                # print(not_empty,'not empty')
                # print(is_empty,'is empty')
                for d in is_empty:
                    # print(particle_path[d,:],d,'particle_path[d,:],d')
                    particle_hawkes[d] = self.smc_weight(jump_array_index[t], jump_array_index[t]+iter_len, i, 
                    particle_path[d,:], empty_para_array,d)
                    # if particle_hawkes[d]>600:
                    #     exit()
                    # print(particle_hawkes[d],'particle_hawkes[d]')
                if len(not_empty) != 0:
                    arr, r_index = np.unique(particle_path[not_empty,:], return_index = True,axis=0)
                    # print(arr,'arr')
                    # print(r_index,'index')
                    r_index_ = np.setdiff1d(np.arange(len(not_empty)), r_index)
                    # print(r_index_,'r_index_')
                    for d in range(len(r_index)):
                        row_index = not_empty[r_index[d]]
                        particle_hawkes[row_index] = self.smc_weight(jump_array_index[t], jump_array_index[t]+iter_len, i, 
                        particle_path[row_index,:], empty_para_array,row_index)
                        # print(particle_hawkes[row_index],row_index,'particle_hawkes[row_index],row_index')
                    for d in range(len(r_index_)):
                        row_index = not_empty[r_index_[d]]
                        pp = particle_path[row_index,:]
                        for dd in range(len(r_index)):
                            # print(arr)
                            # print(arr[dd,:])
                            # print(pp)
                            is_equal = np.all(arr[dd,:]==pp)
                            if is_equal == True:
                                row_index_ = not_empty[r_index[dd]]
                                particle_hawkes[row_index] = particle_hawkes[row_index_]
                                break

                # print(particle_path,'particle path')
                # print(particle_hawkes,'particle hawkes')
            ...
            if particle_path[d,t] == self.c_max:
                ...
                if t == 0:
                    ...
                    for c in self.FCPList_exist[t]:
                        ...
                        empty_para_array[t,c,1,0,d] = empty_para_array[t,c,0,0,d]
                        empty_para_array[t,c,0,1,d] = np.clip(npr.normal(self.eta_prior_mu,self.eta_prior_sigma),self.min,self.max)
                        empty_para_array[t,c,1,1,d] = empty_para_array[t,c,0,1,d] 
                        empty_para_array[t,c,0,2,d] = np.clip(npr.normal(self.zeta_prior_mu,self.zeta_prior_sigma),self.min,self.max)
                        empty_para_array[t,c,1,2,d] = empty_para_array[t,c,0,2,d]

                        
                        # fix eta zeta
                        # empty_para_array[t,c,0,1,d] = 0
                        # empty_para_array[t,c,1,1,d] = 0
                    ...
                    #fix eta
                    # empty_para_array[t,self.c_max,0,1,d] = 0
                        ...
                        if particle_path[d,t-1] == particle_path[d,t]: # empty follow empty
                            ...
                            for c in self.FCPList_exist[t]:
                                ...
                                if c == ori_node:
                                    ...
                                    # print(self.c_max,empty_para_array[t,self.c_max,0,0,d],empty_para_array[t,self.c_max,0,1,d],
                                    # empty_para_array[t,self.c_max,0,2,d],'empty_para_array[t,self.c_max,0,1,d]')
                                    # fix eta zeta
                                ...
                                # print(c,empty_para_array[t,c,0,0,d],empty_para_array[t,c,0,1,d], empty_para_array[t,c,1,2,d],' empty_para_array[t,c,1,2,d]')
    ...
    def sample_scaling(self, para_select):
        ...
        '''
        if para_select == 0:
            new_b = npr.normal(loc = self.b, scale = 5)
            new_b = np.clip(new_b, 0, self.b_max)
        if para_select == 1:
            new_eta = npr.normal(loc = self.eta, scale = 5)
            new_eta = np.clip(new_eta, 0.000001, self.eta_max)
        if para_select == 2:
            new_zeta = npr.normal(loc = self.zeta, scale = 5)
            new_zeta = np.clip(new_zeta, 0.000001, self.zeta_max)


        for i in range(self.d_num):
            for j in range(i+1,self.d_num):
                if i!=j:
                    s_array, r_array, time = self.path_for_hawkes_scaling(self.path_entity[i],self.path_entity[j])
    ...
    def cal_hawkes_likelihood_(self, time__, b, eta, zeta, b_base, t_base, t_trigger):
        ...
        for i in range(len(t_base)):
            q_ = b[i] + np.dot(eta[:i],np.exp(-zeta[:i]*(t_base[i]-t_trigger[:i])))

            q = q + np.log(q_)
        # print(np.dot(b_base,interval_base),'np.dot(b_base,interval_base)')
    ...
    def count_split_merge(self):

        time = np.asarray(self.FCPList_time)
...
def logit(x): # checked
    # if np.ndim(x) == 1:
    #     index = np.where(x<-10)[0]
    #     if len(index)>0:
    #         print(x)
    #         exit()
    # else:
    #     index_1,index_2 = np.where(x<-10)
    #     if len(index_1)>0:
    #         print(x)
    #         exit()
    return 1/(1+np.exp(-x))
